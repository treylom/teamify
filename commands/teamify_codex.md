...
```bash
bash .claude/scripts/setup-teamify-codex.sh
```
...
```bash
AUTO_INSTALL=1 bash .claude/scripts/setup-teamify-codex.sh
```
...
```bash
RUN_PROXY_TEST=1 bash .claude/scripts/setup-teamify-codex.sh
```

---

<mindset priority="HIGHEST">
이 명령어는 **환경 설정만 수행**하고, 팀 구성은 `/teamify`에 위임합니다.

핵심 원리 — **시간차 환경변수 주입**:
1. Leader(현재 프로세스)는 이미 Opus로 실행 중 → process.env 변경 불가
2. `tmux set-environment`로 세션 레벨에 프록시 URL 설정
3. `/teamify` → TeamCreate → Task spawn → 새 pane 생성
...
<teamify_overrides priority="HIGH">
**teamify 위임 시 오버라이드 사항**

teamify의 STEP 2-B (DA 모델 선택)에서 선택지를 다음으로 대체:
- "OFF" — DA 없이 진행
- "ON (Codex) (Recommended)" — GPT-5.3-Codex로 DA (CLIProxyAPI 경유)
- "ON (Opus)" — Anthropic Opus로 DA (Leader와 동일, Anthropic Direct)

**DA 흐름: teamify 원본 그대로 사용 (수정 없음)**

teamify의 DA 2-Phase 흐름을 그대로 따릅니다:
...
Why: teamify의 DA 흐름에서 idle 문제는 개별 리뷰 단계(STEP 7-6)를 생략했기 때문.
teamify를 올바르게 위임하면 DA가 워커 결과마다 리뷰 작업을 받으므로 idle 시간이 짧음.
</teamify_overrides>
...
>
> `/teamify` 워크플로우를 시작합니다...

---

## PHASE 2.5: Agent Office 대시보드 실행

**teamify 위임 전에 Agent Office를 미리 실행합니다.**
...
## PHASE 3: teamify 워크플로우 위임 (전체 클론)

```
Skill("teamify", args: "$ARGUMENTS")
```

표준 teamify STEP 0~9 전체 실행. 아래 항목이 반드시 포함되어야 합니다:

### 필수 포함 항목 (teamify가 자체 실행 — teamify_codex가 개입 X)
...
teamify_codex는 PHASE 0-2.5만 직접 실행하고, STEP 0~9는 teamify에 100% 위임합니다.
teamify의 DA 흐름, progress 업데이트, Agent Office 연동을 절대 생략하거나 간소화하지 마세요.

### Codex 특화 검증 (teamify 위임 후 자동)

- 새 pane은 tmux 세션 env 상속 → `ANTHROPIC_BASE_URL=http://127.0.0.1:8317`
- 팀원 프로세스는 CLIProxyAPI 경유 → Codex 실행

**teamify STEP 2-B DA 모델 선택 오버라이드:**
...
**teamify STEP 7-4 스폰 후 Codex 검증:**
각 팀원 스폰 후, 팀원 pane에서 `ANTHROPIC_BASE_URL` 상속 여부를 확인:
```bash
tmux capture-pane -t {pane_id} -p | head -5
```

teamify가 완료되면 PHASE 4로 진행합니다.

---

## PHASE 4: 정리

teamify 완료 후 (팀 셧다운 시) 프록시 + 환경 정리:
...
1. teamify의 STEP 7-6 (개별 워커 DA 리뷰)이 정상 실행되고 있는지 확인
2. 리드가 워커 결과 수신 시 DA에게 SendMessage를 보내고 있는지 확인
3. 정상 흐름에서는 DA가 워커 결과마다 리뷰 요청을 받으므로 idle 시간이 짧음
4. 그래도 경고가 반복되면 teamify 위임이 제대로 되지 않은 것 → Skill("teamify") 호출 확인
...
현재 세션을 종료하고 새 세션에서 `/teamify_codex`를 실행하세요.
...
| DA idle hook | teamify 원본 흐름(개별 리뷰+종합 리뷰) 따르면 최소화됨. 간소화 시 발생 |
