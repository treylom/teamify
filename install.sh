...
print_header() {
  echo ""
  echo -e "${BOLD}${CYAN}╔══════════════════════════════════════════╗${NC}"
  echo -e "${BOLD}${CYAN}║        teamify installer v${VERSION}         ║${NC}"
  echo -e "${BOLD}${CYAN}╚══════════════════════════════════════════╝${NC}"
  echo ""
}
...
resolve_source() {
  if [ -n "$SCRIPT_DIR" ] && [ -f "$SCRIPT_DIR/commands/teamify.md" ]; then
    SRC="$SCRIPT_DIR"
    info "Installing from local repo / 로컬 저장소에서 설치"
  else
    TEMP=$(mktemp -d)
    info "Cloning teamify... / teamify 다운로드 중..."
    if ! git clone --depth 1 "$REPO" "$TEMP/teamify" 2>/dev/null; then
...
check_prerequisites() {
  ...
  if [ "$missing" -gt 0 ]; then
    ...
    warn "teamify will install but may not work fully without them"
    echo ""
...
install_files() {
  step 3 "Installing teamify files... / teamify 파일 설치 중..."

  # Command
  mkdir -p .claude/commands
  cp "$SRC/commands/teamify.md" .claude/commands/
  success ".claude/commands/teamify.md"
  ...
  success ".claude/skills/teamify-*.md ($skill_count files)"
...
verify_installation() {
  ...
  # Check files exist
  for f in .claude/commands/teamify.md; do
    if [ -f "$f" ]; then
      success "$f exists"
    else
      error "$f missing!"
  ...
  # Check skills
  local skill_count
  skill_count=$(ls .claude/skills/teamify-*.md 2>/dev/null | wc -l)
  if [ "$skill_count" -ge 1 ]; then
    success ".claude/skills/teamify-*.md ($skill_count files)"
  else
    error "No teamify skill files found!"
    errors=$((errors + 1))
  fi
...
print_summary() {
  ...
  echo ""
  echo -e "${GREEN}╔══════════════════════════════════════════════════════╗${NC}"
  echo -e "${GREEN}║  teamify v${VERSION} installed! / 설치 완료!             ║${NC}"
  echo -e "${GREEN}╚══════════════════════════════════════════════════════╝${NC}"
  echo ""
  ...
  echo "    2. Type /teamify / /teamify 입력"
  echo ""
  ...
  echo "    https://github.com/treylom/teamify"
  echo ""
}
